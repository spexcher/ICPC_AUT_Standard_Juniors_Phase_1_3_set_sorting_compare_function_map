...
bool compareFriend(const Friend &f1, const Friend &f2)
{
  int t1 = f1.a + f1.m + f1.s + f1.e;
  int t2 = f2.a + f2.m + f2.s + f2.e;
  if (abs(t1 - t2) > 10)
  {
    return t1 > t2;
  }
...
void solve()
{
  ...
  fo(i, n)
  {
    int tm = v[i].a + v[i].m + v[i].s + v[i].e;
    print(v[i].name << sp << tm << sp << v[i].a << sp << v[i].m << sp << v[i].s << sp << v[i].e);
  }
}